...
// Route: Bulk delete SSH hosts (requires JWT)
// POST /ssh/bulk-delete
router.post(
  "/bulk-delete",
  authenticateJWT,
  async (req: Request, res: Response) => {
    ...
    if (hostIds.length > 100) {
      return res
        .status(400)
        .json({ error: "Maximum 100 hosts allowed per bulk delete" });
    }
    ...
    for (const hostId of hostIds) {
      try {
        ...
        if (isNaN(numericHostId)) {
          results.failed++;
          results.errors.push(`Invalid host ID: ${hostId}`);
          continue;
        }
        ...
        const hostToDelete = await db
          ...
          .from(sshData)
          .where(
            and(eq(sshData.id, numericHostId), eq(sshData.userId, userId)),
          );

        if (hostToDelete.length === 0) {
          results.failed++;
          results.errors.push(`Host ${hostId} not found`);
          continue;
        }
        ...
        results.failed++;
        results.errors.push(
          `Host ${hostId}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    }
...
// Route: Bulk assign tags to SSH hosts (requires JWT)
// POST /ssh/bulk-assign-tags
router.post(
  "/bulk-assign-tags",
  authenticateJWT,
  async (req: Request, res: Response) => {
    const userId = (req as any).userId;
    const { hostIds, tags, mode = "add" } = req.body;
    ...
    if (!Array.isArray(tags) || tags.length === 0) {
      return res
        .status(400)
        .json({ error: "tags array is required and must not be empty" });
    }

    if (!["add", "replace"].includes(mode)) {
      return res
        .status(400)
        .json({ error: "mode must be 'add' or 'replace'" });
    }
    ...
    for (const hostId of hostIds) {
      try {
        ...
        results.success++;
        sshLogger.success(
          `Tags ${mode === "add" ? "added to" : "replaced for"} host ${host.name} by user ${userId}`,
          {
            operation: "bulk_assign_tags",
            userId,
            hostId: numericHostId,
            mode,
            tags: newTags,
    ...
    res.json({
      message: `Bulk tag assignment completed: ${results.success} successful, ${results.failed} failed`,
      success: results.success,
      failed: results.failed,
      errors: results.errors,
    });
...
// Route: Bulk move SSH hosts to folder (requires JWT)
// POST /ssh/bulk-move-to-folder
router.post(
  "/bulk-move-to-folder",
  authenticateJWT,
  async (req: Request, res: Response) => {
    const userId = (req as any).userId;
    const { hostIds, folder } = req.body;
    ...
    for (const hostId of hostIds) {
      try {
        ...
        const host = hosts[0];
        sshLogger.success(
          `Host ${host.name} moved to folder "${folder || "Uncategorized"}" by user ${userId}`,
          {
    ...
    res.json({
      message: `Bulk move to folder completed: ${results.success} successful, ${results.failed} failed`,
      success: results.success,
      failed: results.failed,
      errors: results.errors,
    });
...
// Route: Bulk export SSH hosts (requires JWT and data access)
// POST /ssh/bulk-export
router.post(
  "/bulk-export",
  authenticateJWT,
  requireDataAccess,
  async (req: Request, res: Response) => {
    ...
    if (hostIds.length > 100) {
      return res
        .status(400)
        .json({ error: "Maximum 100 hosts allowed per bulk export" });
    }
    ...
    for (const hostId of hostIds) {
      try {
        ...
        results.hosts.push(exportData);
        results.success++;
        sshLogger.success(`Host ${host.name} exported by user ${userId}`, {
    ...
    res.json({
      message: `Bulk export completed: ${results.success} successful, ${results.failed} failed`,
      success: results.success,
      failed: results.failed,
      hosts: results.hosts,
      errors: results.errors,
