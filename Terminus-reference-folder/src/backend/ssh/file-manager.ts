...
app.post("/ssh/file_manager/ssh/read_file_content", async (req, res) => {
  const { sessionId, filePath, hostId, userId } = req.body;

  if (!sessionId || !filePath) {
    fileLogger.warn("Missing read file content parameters", {
  ...
  const sshConn = sshSessions[sessionId];
  if (!sshConn || !sshConn.isConnected) {
    fileLogger.warn("SSH session not found or not connected for read file content", {
      operation: "read_file_content",
      sessionId,
      isConnected: sshConn?.isConnected,
    });
  ...
  sshConn.client.sftp((err, sftp) => {
    if (err) {
      fileLogger.error("SFTP connection failed for read file content:", err);
      return res.status(500).json({ error: "SFTP connection failed" });
    }

    sftp.stat(filePath, (statErr, stats) => {
      if (statErr) {
        fileLogger.error("File stat failed for read file content:", statErr);
        return res
          .status(500)
          .json({ error: `Cannot access file: ${statErr.message}` });
      }
      ...
      if (!stats.isFile()) {
        fileLogger.warn("Attempted to read non-file", {
          operation: "read_file_content",
          sessionId,
          filePath,
          isFile: stats.isFile(),
        ...
        return res
          .status(400)
          .json({ error: "Cannot read directories or special files" });
      }

      // Limit file size for editor (10MB)
      const MAX_FILE_SIZE = 10 * 1024 * 1024;
      if (stats.size > MAX_FILE_SIZE) {
        fileLogger.warn("File too large for editing", {
          operation: "read_file_content",
          sessionId,
          filePath,
          fileSize: stats.size,
        ...
        return res.status(400).json({
          error: `File too large for editing. Maximum size is ${MAX_FILE_SIZE / 1024 / 1024}MB, file is ${(stats.size / 1024 / 1024).toFixed(2)}MB`,
        });
      }

      sftp.readFile(filePath, (readErr, data) => {
        if (readErr) {
          fileLogger.error("File read failed for read file content:", readErr);
          return res
            .status(500)
            .json({ error: `Failed to read file: ${readErr.message}` });
        }
        ...
        const isBinary = data.some((byte, index) => {
          if (index > 8000) return false; // Check only first 8KB
          return byte === 0 || (byte < 32 && byte !== 9 && byte !== 10 && byte !== 13);
        });

        if (isBinary) {
          fileLogger.warn("Attempted to read binary file as text", {
            operation: "read_file_content",
            sessionId,
            filePath,
          });
          return res.status(400).json({
            error: "Cannot edit binary files. Please use download instead.",
          });
        }
        ...
        fileLogger.success("File content read successfully", {
          operation: "read_file_content",
          sessionId,
          filePath,
          fileName,
...
app.post("/ssh/file_manager/ssh/save_file_content", async (req, res) => {
  const { sessionId, filePath, content, hostId, userId } = req.body;

  if (!sessionId || !filePath || content === undefined) {
    fileLogger.warn("Missing save file content parameters", {
      operation: "save_file_content",
      sessionId,
      hasFilePath: !!filePath,
  ...
  const sshConn = sshSessions[sessionId];
  if (!sshConn || !sshConn.isConnected) {
    fileLogger.warn("SSH session not found or not connected for save file content", {
      operation: "save_file_content",
      sessionId,
      isConnected: sshConn?.isConnected,
    });
  ...
  sshConn.client.sftp((err, sftp) => {
    if (err) {
      fileLogger.error("SFTP connection failed for save file content:", err);
      return res.status(500).json({ error: "SFTP connection failed" });
    }
    ...
    sftp.writeFile(filePath, buffer, (writeErr) => {
      if (writeErr) {
        fileLogger.error("File write failed for save file content:", writeErr);
        return res
          .status(500)
          .json({ error: `Failed to save file: ${writeErr.message}` });
      }

      const fileName = filePath.split("/").pop() || "file";

      fileLogger.success("File content saved successfully", {
        operation: "save_file_content",
        sessionId,
        filePath,
        fileName,
