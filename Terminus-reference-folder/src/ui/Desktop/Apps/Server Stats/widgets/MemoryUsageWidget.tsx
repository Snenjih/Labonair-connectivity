...
import { MemoryStick, AlertCircle } from "lucide-react";
...
interface MemoryUsageWidgetProps {
  metricsData: ServerMetric[];
  latestMetric: ServerMetric | null;
}

export function MemoryUsageWidget({ metricsData, latestMetric }: MemoryUsageWidgetProps) {
  // Prepare chart data
  ...
  const currentUsage = latestMetric?.memoryUsage;
    ...
    <Card>
      ...
      <CardContent>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {currentUsage < 70 ? "Normal" : currentUsage < 90 ? "Elevated" : "High"} usage
        </p>
