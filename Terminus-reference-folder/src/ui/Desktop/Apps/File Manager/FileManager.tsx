...
function FileManagerContent({ initialHost, onClose }: FileManagerProps) {
  ...
  // Right panel state for Commander View
  const [rightPath, setRightPath] = useState(initialHost?.defaultPath || "/");
  const [rightFiles, setRightFiles] = useState<FileItem[]>([]);
  const [rightLoading, setRightLoading] = useState(false);
  const [rightCreateIntent, setRightCreateIntent] = useState<CreateIntent | null>(null);

  // Panel type state (local or remote)
  ...
  // Local files state for left panel (when panel type is "local")
  const [leftLocalPath, setLeftLocalPath] = useState<string>("");
  const [leftLocalFiles, setLeftLocalFiles] = useState<FileItem[]>([]);
  const [leftLocalLoading, setLeftLocalLoading] = useState(false);
  ...
  const loadLocalDirectory = useCallback(
    async (path: string) => {
      setLeftLocalLoading(true);
      setCreateIntent(null);

      try {
        const response = await listLocalFiles(path || "");
        const files = response.files || [];
        setLeftLocalFiles(files);

        // Update the path to the actual resolved path from the backend
        if (response.path && response.path !== path) {
          setLeftLocalPath(response.path);
        }
  ...
  async function handleLocalConfirmCreate(name: string) {
    if (!createIntent) return;

    try {
      if (createIntent.type === "file") {
        await createLocalFile(leftLocalPath, name, "");
        toast.success(t("fileManager.fileCreatedSuccessfully", { name }));
      } else {
        await createLocalFolder(leftLocalPath, name);
        toast.success(t("fileManager.folderCreatedSuccessfully", { name }));
      }

      setCreateIntent(null);
      loadLocalDirectory(leftLocalPath);
    } catch (error: any) {
      console.error("Create failed:", error);
      toast.error(t("fileManager.failedToCreateItem"));
    }
  ...
  async function handleDeleteLocalFiles(files: FileItem[]) {
    ...
    confirmWithToast(
      ...
      async () => {
        try {
          for (const file of files) {
            await deleteLocalItem(file.path, file.type === "directory");
          }
  ...
  async function handleRenameLocalFile(file: FileItem, newName: string) {
    try {
      await renameLocalItem(file.path, newName);
  ...
  async function handleLocalFileDrop(
    ...
    try {
      ...
      for (const file of draggedFiles) {
        try {
          await moveLocalItem(file.path, targetFolder.path);
          successCount++;
        } catch (error: any) {
  ...
  async function handleLocalFileOpen(file: FileItem) {
    if (file.type === "directory") {
  ...
  // Load file manager design setting
  useEffect(() => {
    const loadFileManagerDesign = async () => {
      try {
    ...
    loadFileManagerDesign();
  }, []);

  // Load right panel directory for Commander View
  const loadRightDirectory = useCallback(
    async (path: string) => {
      if (!sshSessionId) {
        console.error("Cannot load right directory: no SSH session ID");
        return;
      }

      setRightLoading(true);
      setRightCreateIntent(null);
      ...
      try {
        ...
        console.error("Failed to load right directory:", error);
        toast.error(
          t("fileManager.failedToLoadDirectory") +
            ": " +
            (error.message || error)
  ...
  useEffect(() => {
    if (fileManagerDesign === "commander" && sshSessionId && rightPath) {
      loadRightDirectory(rightPath);
    }
  }, [fileManagerDesign, sshSessionId, rightPath, loadRightDirectory]);

  // Load local directory when commander view is active and left panel is local
  useEffect(() => {
    if (fileManagerDesign === "commander" && leftPanelType === "local") {
      // Initialize with empty path on first load to get home directory
      if (!leftLocalPath) {
        loadLocalDirectory("");
      } else {
        loadLocalDirectory(leftLocalPath);
  ...
  }, [fileManagerDesign, leftPanelType, leftLocalPath, loadLocalDirectory]);

  const handleRightRefresh = useCallback(() => {
    loadRightDirectory(rightPath);
  }, [rightPath, loadRightDirectory]);

  const handleRightFileOpen = useCallback(
    (file: FileItem) => {
      if (file.type === "directory") {
        setRightPath(file.path);
      } else {
        handleFileOpen(file);
      }
    },
  ...
  async function handleRightConfirmCreate(name: string) {
    if (!rightCreateIntent || !sshSessionId) return;

    try {
      await ensureSSHConnection();

      if (rightCreateIntent.type === "file") {
        await createSSHFile(
          sshSessionId,
          rightPath,
          name,
  ...
  function handleRightCancelCreate() {
    setRightCreateIntent(null);
  }
    ...
    <div className="h-full flex flex-col bg-dark-bg">
      ...
      <div className="flex-1 flex" {...dragHandlers}>
        {fileManagerDesign === "commander" ? (
          /* Commander View - Two-panel Orthodox layout */
          <CommanderView
            // Panel types
            leftPanelType={leftPanelType}
            rightPanelType={rightPanelType}

            // Left Panel (conditional based on panel type)
            leftFiles={leftPanelType === "local" ? leftLocalFiles : filteredFiles}
            leftPath={leftPanelType === "local" ? leftLocalPath : currentPath}
            leftLoading={leftPanelType === "local" ? leftLocalLoading : isLoading}
            onLeftPathChange={leftPanelType === "local" ? setLeftLocalPath : setCurrentPath}
            onLeftRefresh={leftPanelType === "local" ? () => loadLocalDirectory(leftLocalPath) : handleRefreshDirectory}
            onLeftFileOpen={leftPanelType === "local" ? handleLocalFileOpen : handleFileOpen}

            // Right Panel (stays remote for now)
            rightFiles={rightFiles}
            rightPath={rightPath}
            rightLoading={rightLoading}
            onRightPathChange={setRightPath}
            onRightRefresh={handleRightRefresh}
            onRightFileOpen={handleRightFileOpen}
            ...
            onRename={(file, newName) => {
              if (leftPanelType === "local") {
                handleRenameLocalFile(file, newName);
              } else {
                handleRenameConfirm(file, newName);
              }
            }}
            ...
            leftCreateIntent={createIntent}
            rightCreateIntent={rightCreateIntent}
            onLeftConfirmCreate={leftPanelType === "local" ? handleLocalConfirmCreate : handleConfirmCreate}
            onLeftCancelCreate={handleCancelCreate}
            onRightConfirmCreate={handleRightConfirmCreate}
            onRightCancelCreate={handleRightCancelCreate}
          />
        ) : (
          /* Explorer View - Traditional single-panel layout */
          <>
