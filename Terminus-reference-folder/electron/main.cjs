...
const fs = require("fs");
const os = require("os");
const { startBackend, stopBackend } = require("./start-backend.cjs");
...
const REPO_OWNER = "Snenjih";
const REPO_NAME = "Terminus";
  ...
  try {
    ...
    const response = await fetch(`${GITHUB_API_BASE}${endpoint}`, {
      headers: {
        ...
        "User-Agent": "TerminusElectronUpdateChecker/1.0",
    ...
    if (!response.ok) {
      ...
      let errorDetails = { status: response.status, endpoint };

      if (response.status === 404) {
        errorMessage =
          "GitHub repository or release not found. Please verify the repository exists and has releases.";
        errorDetails.reason = "not_found";
        errorDetails.repository = `${REPO_OWNER}/${REPO_NAME}`;
        errorDetails.suggestion =
          "Check if the repository name is correct and if any releases have been published.";
      } else if (response.status === 403) {
        errorMessage =
          "GitHub API rate limit exceeded. Please try again later.";
        errorDetails.reason = "rate_limit";
    ...
    if (error instanceof Error) {
      if (error.message.includes("GitHub API error")) {
        // Already formatted error, just throw
...
ipcMain.handle("test-server-connection", async (event, serverUrl) => {
  try {
      ...
      if (response.ok) {
        ...
        if (
          ...
          return {
            success: false,
            error:
              "Server returned HTML instead of JSON. This does not appear to be a Terminus server.",
          };
        }
    ...
    return {
      success: false,
      error:
        "Server is not responding or does not appear to be a valid Terminus server. Please ensure the server is running and accessible.",
    };
  } catch (error) {
    return { success: false, error: error.message };
  }
